...
import com.localassistant.tools.Tool
...
class ToolConfirmationTest {
    ...
    // ═══════════════════════════════════════════════════════
    //  A. Tool requiresConfirmation classification  –  200+ scenarios
    // ═══════════════════════════════════════════════════════

    private val toolConfirmationMap = mapOf(
        "calendar" to true,       // CalendarTool writes events → requires confirmation
        "memory" to true,        // MemoryTool saves data → requires confirmation
        "task" to true,          // TaskTool creates/completes tasks → requires confirmation
        "clipboard" to true,     // ClipboardTool writes → requires confirmation
        "contacts_search" to false, // ContactsTool is read-only → no confirmation
        "system_info" to false,  // SystemInfoTool is read-only → no confirmation
        "web_search" to false,   // WebSearchTool is read-only (but needs network) → no confirmation
        "alarm_set" to true,     // AlarmTool sets alarms → requires confirmation
        "file_read" to false,    // FileReaderTool is read-only → no confirmation
        "email_draft" to true,   // EmailDraftTool creates drafts → requires confirmation
        "sms_draft" to true      // SmsDraftTool creates drafts → requires confirmation
    )

    /** A1 – 11 tools × 20 checks = 220 */
    @Test
    fun a1_toolConfirmationClassification() {
        ...
        assertTrue("Expected ≥220, got $count", count >= 220)
    }

    // ═══════════════════════════════════════════════════════
    //  B. Memory tool confirmation  –  100+ scenarios
    // ═══════════════════════════════════════════════════════
    ...
    private val memoryContents = listOf("Prefers dark mode", "Allergic to peanuts", "Birthday June 5",
        "", " ", "A".repeat(5000), "<script>alert(1)</script>", "你好世界")
    private val memoryQueries = listOf("preferences", "allergies", "", "dark mode", "<script>")

    /** B1 – 2 actions × 8 contents × 5 queries × 2 confirms = 160 */
    @Test
    fun b1_memoryToolConfirmation() {
                ...
                for (query in memoryQueries) {
                    ...
                    for (confirmAction in listOf("confirm", "dismiss")) {
                        when (confirmAction) {
                            ...
                            "dismiss" -> {
                                if (requiresConfirm) {
                                    // User dismisses → tool does NOT execute
                                    // No ToolResult should be produced
                                    val wasDismissed = true
        ...
        assertTrue("Expected ≥160, got $count", count >= 160)
    }

    // ═══════════════════════════════════════════════════════
    //  C. Task tool confirmation  –  100+ scenarios
    // ═══════════════════════════════════════════════════════
    ...
    /** C1 – 3 actions × 5 titles × 3 filters × 3 confirms = 135 */
    @Test
    fun c1_taskToolConfirmation() {
        ...
        assertTrue("Expected ≥135, got $count", count >= 135)
    }
    ...
    /**
     * D1 – Test Case 5: Tool Confirmation
    ...
    /**
     * D2 – Task creation shows confirmation
     */
    @Test
    ...
    /**
     * D3 – Read-only tools do NOT show confirmation
     */
    @Test
    ...
    /**
     * D4 – Confirming tool executes successfully
     */
    @Test
    ...
    /**
     * D5 – Dismissing tool does not execute
     */
    @Test
