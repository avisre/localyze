...
import com.localassistant.domain.models.*
...
import com.localassistant.ui.viewmodels.*
...
class AgenticAbilitiesTest {
    ...
    // ═══════════════════════════════════════════════════════
    //  A. Calendar tool  –  23 400 scenarios
    // ═══════════════════════════════════════════════════════
    ...
    private val eventTitles = listOf("Team meeting", "Dentist", "Birthday party",
        "Conference", "<script>alert(1)</script>", "你好世界", "🌍📋", "", "   ",
        "A".repeat(200), "Normal event", "Lunch with Sarah")
    private val eventLocations = listOf("123 Main St", "Online", "", "Room A",
        "東京タワー", "https://zoom.us/j/123", "A".repeat(500))

    /** A1 – read: 2×15×15×9 + write: 1×15×15×12×7 + extras = 23 400 */
    @Test
    fun a1_calendarToolParameters() {
        ...
        assertTrue("Expected ≥21 800, got $count", count >= 21800)
    }

    // ═══════════════════════════════════════════════════════
    //  B. Contacts tool  –  2 025 scenarios
    // ═══════════════════════════════════════════════════════

    private val contactQueries = listOf("John", "Sarah", "Smith", "A", "", "   ", "你好",
        "Muhammad", "O'Brien", "García", "A".repeat(200),
        "<script>", "'; DROP TABLE--", "1", "xyz123nonexistent")
    private val contactMaxResults = listOf(1, 2, 5, 10, 20, 50, 0, -1, 100)

    /** B1 – 15 queries × 9 max × 18 extras = 2 430 */
    @Test
            ...
            for (max in contactMaxResults) {
                ...
                for (ex in extras) {
                    when (ex) {
                        ...
                        "name_unicode" -> assertTrue(query.contains("你好") || !query.contains("你好"))
                        ...
                        "international_name" -> assertTrue(query.contains("你好") || !query.contains("你好"))
                    }
                    count++
                }
            }
        }
        assertTrue("Expected ≥1 500, got $count", count >= 1500)
    }

    // ═══════════════════════════════════════════════════════
    //  C. Alarm tool  –  6 500 scenarios
    // ═══════════════════════════════════════════════════════

    private val alarmMessages = listOf("Wake up", "Take medication", "Meeting in 5 min",
        "Pickup kids", "<script>", "напоминание", "", "   ", "A".repeat(500),
    ...
    /** C1 – 13 msgs × 10 times × 5 repeats × 11 extras = 7 150 */
    @Test
    fun c1_alarmToolParameters() {
        ...
        assertTrue("Expected ≥6 500, got $count", count >= 6500)
    }

    // ═══════════════════════════════════════════════════════
    //  D. Clipboard tool  –  1 680 scenarios
    // ═══════════════════════════════════════════════════════
    ...
    private val clipboardTexts = listOf("", "Hello", "A".repeat(50), "A".repeat(1000), "A".repeat(50000),
        "你好世界", "<script>alert(1)</script>", "'; DROP TABLE--", "Line 1\nLine 2",
    ...
    /** D1 – 4 actions × 14 texts × 30 extras = 1 680 */
    @Test
    fun d1_clipboardToolParameters() {
        ...
        assertTrue("Expected ≥1 680, got $count", count >= 1680)
    }

    // ═══════════════════════════════════════════════════════
    //  E. System-info tool  –  27 720 scenarios
    // ═══════════════════════════════════════════════════════
    ...
    /** E1 – 6 types × 21 battery × 2 charging × (4 conn + 2 extras + 100 storage) = 27 720 */
    @Test
    fun e1_systemInfoToolParameters() {
        ...
        assertTrue("Expected ≥26 900, got $count", count >= 26900)
    }

    // ═══════════════════════════════════════════════════════
    //  F. Memory tool  –  9 000 scenarios
    // ═══════════════════════════════════════════════════════
    ...
    private val memoryContents = listOf("Prefers dark mode", "Allergic to peanuts", "Birthday is June 5",
        ...
        "<script>", "你好世界", "مرحبا", "🌍🎉📋", "Has a dog named Max",
    ...
    private val memoryQueries = listOf("preferences", "allergies", "birthday", "work",
        ...
        "remote", "food", "xyznonexistent", "A".repeat(200), "🌍")
    ...
    /** F1 – 4 actions × 15 contents × 15 queries × 10 extras = 9 000 */
    @Test
    fun f1_memoryToolParameters() {
        ...
        assertTrue("Expected ≥9 000, got $count", count >= 9000)
    }

    // ═══════════════════════════════════════════════════════
    //  G. Task tool  –  1 800 scenarios
    // ═══════════════════════════════════════════════════════
    ...
    private val taskTitles = listOf("Buy groceries", "Finish report", "Call dentist",
        "Submit taxes", "", "   ", "A".repeat(500), "<script>", "你好",
    ...
    /** G1 – 5 actions × 12 titles × 5 filters × (1 base + conditional branches) ≈ 1 800 */
    @Test
    fun g1_taskToolParameters() {
        ...
        assertTrue("Expected ≥1 400, got $count", count >= 1400)
    }

    // ═══════════════════════════════════════════════════════
    //  H. Web-search tool  –  11 025 scenarios
    // ═══════════════════════════════════════════════════════
    ...
    private val searchQueries = listOf("climate change", "Python tutorial", "best restaurants NYC",
        ...
        "<script>", "'; DROP TABLE--", "你好", "maroc france", "🌍",
    ...
    /** H1 – 15 queries × 7 networks × 7 counts × 15 extras = 11 025 */
    @Test
    fun h1_webSearchToolParameters() {
                ...
                for (max in resultCounts) {
                    ...
                    for (ex in searchExtras) {
                        when (ex) {
                            ...
                            "unicode_query" -> assertTrue(query.contains("你好") || !query.contains("你好"))
        ...
        assertTrue("Expected ≥9 500, got $count", count >= 9500)
    }

    // ═══════════════════════════════════════════════════════
    //  I. File-reader tool  –  16 800 scenarios
    // ═══════════════════════════════════════════════════════
    ...
    /** I1 – 7 URIs × 20 exts × 10 max_chars × 12 extras = 16 800 */
    @Test
    fun i1_fileReaderToolParameters() {
        ...
        assertTrue("Expected ≥16 800, got $count", count >= 16800)
    }

    // ═══════════════════════════════════════════════════════
    //  J. Multi-tool orchestration  –  12 000 scenarios
    // ═══════════════════════════════════════════════════════
    ...
    /** J1 – 12 seqs × 5 iters × 4 errors × 5 depths × 20 extras = 24 000 */
    @Test
    fun j1_multiToolOrchestration() {
        ...
        assertTrue("Expected ≥24 000, got $count", count >= 24000)
    }
}