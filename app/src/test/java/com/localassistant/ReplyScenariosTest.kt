...
class ReplyScenariosTest {
    ...
    // 30 edge conditions per combo → 6×12×18×30 = 38 880
    ...
    /** A – 6 modes × 12 inputs × 18 states × 30 edges = 38 880 */
    @Test
    fun a1_chatUiState_transitions() {
                ...
                for (state in stateVariants) {
                    ...
                    for (edge in edges) {
                        val mutated = when (edge) {
                            ...
                            "unicode_emoji_input" -> ui.copy(streamingText = "🌍🎉")
                            "chinese_input" -> ui.copy(streamingText = "你好")
                            "arabic_input" -> ui.copy(streamingText = "مرحبا")
        ...
        assertTrue("Expected ≥38 880, got $count", count >= 38880)
    }
    ...
    /** B – 9 states × 21 percents × 11 errors × 2 retry × 5 extras = 20 790 */
    @Test
    fun b1_onboardingStateMachine() {
        ...
        assertTrue("Expected ≥20 790, got $count", count >= 20790)
    }

    // ── C. Settings toggle grid ────────────────────────────
    ...
    /** C – 6 toggles × 2 states × 20 queries × 2 dialogs × 10 ops = 4 800 */
    @Test
    fun c1_settingsToggleGrid() {
        ...
        assertTrue("Expected ≥4 800, got $count", count >= 4800)
    }

    // ── D. Capability-mode selection ───────────────────────
    ...
    /** D – 6 modes × 20 questions × 25 edges = 3 000 */
    @Test
    fun d1_capabilityModeSelection() {
            ...
            for (q in questions) {
                ...
                for (edge in edgeOps) {
                    val result = when (edge) {
                        ...
                        "unicode_q" -> Message(conversationId = 1, role = MessageRole.USER, content = "你好مرحبا🌍").let { assertNotNull(it); state }
        ...
        assertTrue("Expected ≥3 000, got $count", count >= 3000)
    }

    // ── E. Message model validation ────────────────────────

    /** E – 4 roles × 8 lengths × 6 images × 2 audio × 2 think × 2 tool × 5 extras = 7 680 */
    @Test
    fun e1_messageModelValidation() {
        ...
        assertTrue("Expected ≥7 680, got $count", count >= 7680)
    }

    // ── F. Streaming / token lifecycle ─────────────────────

    /** F – 6 events × 12 texts × 9 tools × 2 errors × 5 extras = 6 480 */
    @Test
    fun f1_streamingTokenLifecycle() {
        ...
        assertTrue("Expected ≥6 480, got $count", count >= 6480)
    }

    // ── G. Audio recording lifecycle ───────────────────────

    /** G – 10 elapsed × 7 amps × 10 durations × 3 states × 6 extras = 12 600 */
    @Test
    fun g1_audioRecordingLifecycle() {
        ...
        assertTrue("Expected ≥12 600, got $count", count >= 12600)
    }

    // ── H. Model-load & download states ─────────────────────

    /** H – 4 states × 51 progress × 9 bytes × 8 extras = 14 688 */
    @Test
    fun h1_modelLoadAndDownloadStates() {
        ...
        assertTrue("Expected ≥14 688, got $count", count >= 14688)
    }

    // ── I. Conversation persistence ────────────────────────

    /** I – 6 modes × 10 titles × 7 msg counts × 2 pinned × 8 extras = 6 720 */
    @Test
    fun i1_conversationPersistence() {
        var count = 0
        for (mode in modes) {
            for (title in listOf("New Chat", "My Conversation", "A".repeat(50),
                "你好", "🌍📋", "<script>", "", "   ", "a", "Long title xyz")) {
        ...
        assertTrue("Expected ≥6 720, got $count", count >= 6720)
    }
}